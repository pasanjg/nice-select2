...
NiceSelect.prototype._renderSelectedItems = function () {
  if (this.multiple) {
    var selectedHtml = document.createElement('span');
    ...
    if (this.config.showSelectedItems || window.getComputedStyle(this.dropdown).width === 'auto' || this.selectedOptions.length < 2) {
      this.selectedOptions.forEach((option) => {
        ...
        current.innerHTML = option.data.text;
        current.addEventListener("click", () => this._onItemClicked(option));
        selectedHtml.appendChild(current);
      });

      selectedHtml.innerHTML = selectedHtml.innerHTML === "" ? this.placeholder : selectedHtml.innerHTML;
    } else {
      selectedHtml.innerHTML = this.selectedOptions.length + ' ' + this.selectedtext;
    }

    this.dropdown.querySelector(".multiple-options").appendChild(selectedHtml);
  } else {
